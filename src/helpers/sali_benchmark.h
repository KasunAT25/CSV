...
long benchmark_lookup_sali(std::vector<KEY_TYPE>& lookups, sali::SALI<KEY_TYPE, PAYLOAD_TYPE> & index){
    ...
    for(int i = 0; i < size; i++){
         ...
         auto payload = index.at(lookups[i], p);
         //auto payload = index.exists(lookups[i]);
...
std::vector<long> perform_benchmark(sali::SALI<KEY_TYPE, PAYLOAD_TYPE> & index, std::vector<KEY_TYPE>& data, std::vector<KEY_TYPE>& lookups, int num){
    std::vector<long> measurements;
     ...
     for (int i = 0; i < num; i++){
        flushCache();
        long time = benchmark_lookup_sali(lookups, index);
