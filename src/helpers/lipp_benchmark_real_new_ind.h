...
// }
long benchmark_lookup_lipp(KEY_TYPE lookup, LIPP<KEY_TYPE, PAYLOAD_TYPE> & index, int num, int idx){
    //Add the vectors
...
std::vector<long> perform_benchmark(LIPP<KEY_TYPE, PAYLOAD_TYPE> & index, std::vector<KEY_TYPE>& data, std::vector<KEY_TYPE>& lookups, int num){
    ...
    //std::cout << "Out 1" << std::endl;
     ...
     for (int i = 0; i < lookups.size(); i++){
        long time = benchmark_lookup_lipp(lookups[i], index, num, i);
