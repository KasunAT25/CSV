...
public:
    ...
    void insert(const T &key, const P &value) {
      ...
      bool state = insert_tree2(key, value);
    ...
    int get_first_key_from_node(Node *node, T *key) {
      ...
      for (int i = 0; i < node->num_items; i++) {
        ...
        //I added ==3 if it is a poisoned value as well.
          ...
          if (node->items[pos].entry_type == 0) { // 0 means empty
            ...
            if (node->items[pos].comp.data.key != key) {
              if (node->last_adjust_type == 3) {
                ...
                std::cout <<node->items[pos].entry_type  << std::endl;
                //std::cout <<node->is_two  << std::endl;
                //std::cout <<node->poi_node << std::endl;
                std::cout << node->items[pos].comp.data.key << " " << key << std::endl;
...
#if READ_EVOLVE
      if (likely(path_size <= 2 || !pq_distribution(getGen()) || path[1]->last_adjust_type == 2)) {
        return ret;
      }
      std::cout << "Evolving 1 " << std::endl;
...
// return that all specializations of this allocator are interchangeable
    ...
    Node* build_tree_two2(T key1, P value1, bool real1, T key2, P value2, bool real2,long double _speed, long _time, int _type)
    {
        if (key1 > key2) {
            std::swap(key1, key2);
            std::swap(value1, value2);
    ...
    Node *
    build_tree_bulk2(std::vector <T> *_keys, std::vector <P> *_values, int _size, std::vector <T> *_keys_real, int _size_real, bulk_args &args) {
// #if READ_EVOLVE
//       args._type=2;
//       std::cout << "Evolving 2 " << std::endl;
//       return build_tree_bulk_two_model(_keys, _values, args);
// #else
//       return build_tree_bulk_fmcd2(_keys, _values, args);
// #endif
      return build_tree_bulk_fmcd2(_keys, _values, _size, _keys_real,_size_real, args);
      /*if (USE_FMCD) {
        return build_tree_bulk_fmcd(_keys, _values, _size);
      } else {
        return build_tree_bulk_fast(_keys, _values, _size);
    ...
    /// poi bulk build, _keys must be sorted in asc order.
    Node *
    poi_build_tree_bulk(T *_keys, P *_values, int _size, T* _keys_real, int _size_real, bulk_args &args) {

      // return poi_build_tree_bulk_fmcd(_keys, _values, _size,  _keys_real, _size_real,args);

#if READ_EVOLVE
      args._type = 2;
      //std::cout << "Evolving poi " << std::endl;
      //return build_tree_bulk_two_model(_keys, _values, args);
      //return poi_build_tree_bulk_fmcd(_keys, _values, _size,  _keys_real, _size_real,args);
      return poi_build_tree_bulk_two_model(_keys, _values, _size,  _keys_real, _size_real,args);
#else
      return poi_build_tree_bulk_fmcd(_keys, _values, _size,  _keys_real, _size_real,args);
      //return build_tree_bulk_fmcd(_keys, _values, args);
#endif
    ...
    // I need to modify this to support my one. Currently it takes new poisoned points as real points
    //Why are they using this in the first place. 
    // Why use 2 models.
    Node *
    ...
    //For read evolve one
    Node* poi_build_tree_bulk_two_model(T* _keys, P* _values, int _size, T* _keys_real, int _size_real, bulk_args &args)
    {
        ...
        while (!s.empty()) {
            ...
            if (end - begin == 2) {
                ...
                bool num2 = std::find(_keys_real, _keys_real + _size_real, _keys[begin+1]) != _keys_real + _size_real;
                    ...
                    //std::cout <<  _keys[begin] <<  " " <<  _keys[begin+1] <<  " " ;
                Node* _ = build_tree_two2(_keys[begin], _values[begin], num1, _keys[begin+1], _values[begin+1], num2, speed, _time, _type);
                memcpy(node, _, sizeof(Node));
                delete_nodes(_, 1);
                

                // if(std::find(_keys_real, _keys_real + _size_real, _keys[begin]) != _keys_real + _size_real && 
                // std::find(_keys_real, _keys_real + _size_real, _keys[begin+1]) != _keys_real + _size_real){

                //     //std::cout <<  _keys[begin] <<  " " <<  _keys[begin+1] <<  " " ;
                //     Node* _ = build_tree_two2(_keys[begin], _values[begin], _keys[begin+1], _values[begin+1],speed, _time, _type);
                //     memcpy(node, _, sizeof(Node));
                //     delete_nodes(_, 1);
                // }
                ...
                int id = 0;
                  ...
                  //std::cout << "all keys " << keys[0] <<std::endl;
                //     //std::cout << "item " << item_i <<std::endl;
                //     std::cout << "is two " << node->is_two <<std::endl;
                //     std::cout << "size " << node->num_items <<std::endl;
                //     std::cout << "a1 " << node->model.a1 <<std::endl;
                //     std::cout << "b1 " << node->model.b1 <<std::endl;
                //     std::cout << "type " << node->model.type <<std::endl;
                //     std::cout << "location" << node->model.a1*static_cast<long double>(keys[0]) + node->model.b1  <<std::endl;
                // for(int i = 0 ; i < _size; i++){
                //   std::cout <<  _keys[i] <<" ";
                // }
                 //std::cout << "===== " << std::endl;

                 //Add a part to it here that will only store those that have no clashes / OR Store all including the clashes.

                // for (int item_i = PREDICT_POS(node, keys[0]), offset = 0; offset < size; ) {
                //     //std::cout << "================" <<  std::endl;
                //     node->poi_node = 1;

                //     node->items[item_i].entry_type = 0;
                ...
                //==============
                //Next edit this so that IF there are crashes then no need to repoison / data is not lost
              //   for (int item_i = PREDICT_POS(node, keys[0]), offset = 0;
              //  offset < size;) {
              //     int next = offset + 1, next_i = -1;
              ...
              //       }
              //       else{
              //         node->items[item_i].entry_type = 3;
              //       }
              //       //node->items[item_i].entry_type = 2;
              //       node->items[item_i].comp.data.key = keys[offset];
              //       node->items[item_i].comp.data.value = values[offset];
                ...
                for (int item_i = PREDICT_POS(node, keys[0]), offset = 0; offset < size; ) {
                    ...
                    if (next == offset + 1) {
                        ...
                        if(real_prev){
                            ...
                            node->items[item_i].comp.data.value = values[offset];
                            //node->size++;
                            //std::cout <<  node->poi_size <<  " POISHERE ";
                            //node->poi_size++;
                        }
    ...
    Node *
    build_tree_bulk_fmcd2(std::vector <T> *_keys, std::vector <P> *_values, int _size,std::vector <T> *_keys_real, int _size_real, bulk_args &args) {

      //int _size = args._size;
      T *keys_real = &((*_keys_real)[0]);
      long double _speed = args._speed;
      uint64_t _time = args._time;
      int _type = args._type;
      //std::cout<<"build_tree_bulk_fmcd "<<std::to_string(args.ratio)<<std::endl;
        ...
        if (end - begin == 2) {
          ...
          const int size = end - begin;
                ...
                // In fact, it has only a negligible impact of the performance.
                {
                    ...
                    //     node->model.a1 = 1.0 / Ut;
                    //     node->model.b1 = (L - node->model.a1 * (static_cast<long double>(keys[size - 1 - D]) +
              ...
              //===================
                for (int item_i = PREDICT_POS(node, keys[0]), offset = 0; offset < size; ) {
                    ...
                    if(std::find(keys_real, keys_real + _size_real, keys[offset]) != keys_real + _size_real){
                            real_prev = true;
                            count++;
                            id = offset;
                        }
                    ...
                    while (next < size) {
                        ...
                        //If it is JUST poisoned then do not create a child. If it also has original data THEN create the child.
                        real = false;
                        if(std::find(keys_real, keys_real + _size_real, keys[next]) != keys_real + _size_real){
                                real = true;   
                                //idx++;
                            }
                        if (next_i == item_i) {
                    ...
                    if (next == offset + 1) {
                        ...
                        if(count > 1){
                            ...
                            if(std::find(keys_real, keys_real + _size_real, keys[id]) != keys_real + _size_real){
                               // BITMAP_CLEAR(node->none_bitmap, item_i);
                               //std::cout <<  keys[id] <<  " ";
                                node->items[item_i].entry_type = 2;
                                node->items[item_i].comp.data.key = keys[id];
    ...
    //Poison and create one model
     Node* poi_build_tree_bulk_fmcd(T* _keys, P* _values, int _size, T* _keys_real, int _size_real, bulk_args &args)
    {
      //std::cout<< "adjusting " << std::endl;
      //int _size = args._size;
      long double _speed = args._speed;
    ...
    void dfs3(Node *node, std::vector <T> *keys,std::vector <T> *keys_real, std::vector <P> *values) {
      //int begin = begin;
      // for (int i = 0; i < node->num_items; i++) { // the i-th entry of the node now
      //   if (node->items[i].entry_type == 2 || node->items[i].entry_type == 3) { // means it is a data
      ...
      //     }
      //   } else if (node->items[i].entry_type == 1) {
      //     dfs3(node->items[i].comp.child, keys, keys_real, values, begin, p);
      //   }
      // }
      for (int i = 0; i < node->num_items; i++) { // the i-th entry of the node now
        if (node->items[i].entry_type == 2 || node->items[i].entry_type == 3) { // means it is a data
          ...
          if(node->items[i].entry_type == 2){
            keys_real->push_back(node->items[i].comp.data.key);
          }
        } else if (node->items[i].entry_type == 1) {
          dfs2(node->items[i].comp.child, keys, values);
        }
      ...
      //                   if(node->items[i].entry_type == 2){
      //                       keys_real[p2] = node->items[i].comp.data.key;
      //                       p2++;
      //                   }
      //               } else {
    ...
    std::pair<int, int> scan_and_destory_tree3(
        Node *_subroot, std::vector <T> **keys,std::vector <T> **keys_real, std::vector <P> **values, // keys here is ptr to ptr
        bool destory = true) {
      ...
      *keys_real = new std::vector<T>();
      (*keys_real)->reserve(_subroot->build_size);

      dfs3(_subroot, *keys, *keys_real, *values) ;
      
      const int ESIZE = (*keys)->size();
      const int ESIZE_real = (*keys_real)->size();
      //std::cout << "dfs ESIZE: " << std::to_string(ESIZE) << std::endl;
      //std::cout << "dfs ESIZE: "<< std::endl;
      return std::make_pair(ESIZE, ESIZE_real);
      //return ESIZE;
    } // end scan_and_destory3

    int scan_and_destory_tree_poi(
        Node *_subroot, std::vector <T> **keys, std::vector <P> **values, // keys here is ptr to ptr
        bool destory = true) {
        ...
        while (!s.empty()) {
            ...
            for (int i = 0; i < node->num_items; i ++) {
                //if (BITMAP_GET(node->none_bitmap, i) == 0) {
                    if (node->items[i].entry_type == 2) {
                        //std::cout << "Here scan3 " << begin <<" " << SHOULD_END_POS<< std::endl;
                        keys[begin] = node->items[i].comp.data.key;
                        values[begin] = node->items[i].comp.data.value;
                        begin ++;
    ...
    bool insert_tree(const T &key, const P &value) {
      ...
      for (Node *node = root;;) {
        ...
        //Try to get the level 2 key
        int pos = PREDICT_POS(node, key);
        versionItem = node->items[pos].readLockOrRestart(needRestart);
        if (needRestart)
          goto restart;

        //Added the poisoning too  
        if (node->items[pos].entry_type == 0 || node->items[pos].entry_type == 3 ) // 0 means empty entry
        {
      ...
      if (node_prob_cur != nullptr) {
        ...
        // to decide the size after getting the locks
        std::vector <P> *values; // scan_and_destroy will fill up the keys/values
        //std::cout << "Here " << node_prob_cur->size << " pois " << node_prob_cur->poi_size << std::endl;
#if COLLECT_TIME
    ...
    //My temp poisoning 
     ...
     // Node* insert_tree(Node *_node, const T &key, const P &value) {
    bool insert_tree2(const T &key, const P &value) {
      int restartCount = 0;
      restart:
      if (restartCount++)
        yield(restartCount);
      ...
      if (node_prob_cur != nullptr) {
        ...
        std::vector <T> *keys_real;

        //std::cout << "Here " << node_prob_cur->size << " pois " << node_prob_cur->poi_size << std::endl;
        // const int ESIZE = node_prob_cur->size;
        // const int ESIZE_poi = node_prob_cur->poi_size;
        // T* keys = new T[ESIZE];   // make it be a ptr here because we will let scan_and_destroy
        // // to decide the size after getting the locks
        // P* values = new P[ESIZE]; // scan_and_destroy will fill up the keys/values

        // T* keys_real = new T[ESIZE];
        //std::cout << "Here_second " <<std::endl;

#if COLLECT_TIME
        auto start_time_scan = std::chrono::high_resolution_clock::now();
#endif
        if (prev_size < 64 || true) {
          int t_size = count_tree_size(node_prob_cur);
          if (t_size < 64 || true) {
            return true;
          }
        ...
        //std::cout << "Here_third " <<std::endl;
        std::pair<int, int> result = scan_and_destory_tree3(
            node_prob_cur, &keys,&keys_real, &values);
        //std::cout << "Here_fourth " <<std::endl;
        int numKeysCollected = result.first; // pass the (address) of the ptr

        // int numKeysCollected = scan_and_destory_tree(
        //     node_prob_cur, &keys, &values);
        int size = result.first;
        int size_real = result.second;
        if (numKeysCollected < 0) {
          return true;
        }
#if COLLECT_TIME
        ...
        Node *new_node = build_tree_bulk2(keys, values, size, keys_real,size_real, args);

        //Node *new_node = build_tree_bulk(keys, values, args);

        //Node *new_node = poi_build_tree_bulk(keys, values, numKeysCollected, keys_real,size_real, args);
    ...
    Node* poison_bulk_load_real(Node* _node, Node* node_poi, const T& key, const P& value, T* keys,P* values, int ESIZE_POI, T* keys_real, int _size_real)
    {
        ...
        // path[path_size] = node;
        // path_size++;
        //std::cout<< "Finding node paths " << std::endl;
        while (true) {
            
            //I try to get a key in the level 2
            int pos = PREDICT_POS(node, key);
            path[path_size] = node;
            //std::cout<< pos <<" , ";
        ...
        for (int i = 0; i < 1; i ++) {
            ...
            int prev_size = node->build_size;
            uint64_t prev_build_time = node->build_time;
            long double prev_speed = node->speed;
        ...
        // const int ESIZE = node->size; //race here
            ...
            if (true) {
                ...
                //Double check if ESIZE should be before or after
                long double speed = (long double) (ESIZE_POI - prev_size) / (cur_time - prev_build_time + 1);
        
                bulk_args args = {ESIZE_POI, speed, cur_time, 1, speed / prev_speed};
                // T* keys2 = new T[ESIZE];
                // P* values2 = new P[ESIZE];

                std::vector <T> *keys2;
                std::vector <P> *values2;
                
                #if COLLECT_TIME
                auto start_time_scan = std::chrono::high_resolution_clock::now();
                #endif
                scan_and_destory_tree2(node, &keys2, &values2);
                #if COLLECT_TIME
                auto end_time_scan = std::chrono::high_resolution_clock::now();
                auto duration_scan = end_time_scan - start_time_scan;
                ...
                new_node = poi_build_tree_bulk(keys, values, ESIZE_POI,keys_real, _size_real, args);
                //new_node->last_adjust_type = 1;
                //std::cout<< "NOT SAME" <<std::endl;
                #if COLLECT_TIME
                auto end_time_build = std::chrono::high_resolution_clock::now();
                auto duration_build = end_time_build - start_time_build;
