...
//========
// IF not in hull
// WHILE max poison reached OR we can add the new point
    // Add a poisoning key
    // Do the above calculations
    // IF the new point can now be added
    // Stop the poisoning
    // NO NEED TO CHECK IF OTHERS ARE BAD SINCE THIS WILL IMPROVE THE PERFORMANCE ANYWAY
    // I might have to redo the rectangle calculations
    //CHECK how to do the resizing, how to do the new rectangle, add the poisoning keys
// 
// 
// 
...
public:
    ...
    bool add_point(const X &x, const Y &y, int level) {
        ...
        if (outside_line1 || outside_line2) {
            points_in_hull = 0;
            //This is where I need to add the poisoning to
            if(level == 1 && poisoning_point){
                //do the poisoning
                //Make sure to add that point into the convex hull
                //Need to 
                // CHECK THE RECTANGLE
                // CHECK THE LOWER
                // UPPER
                // MIN
                // MIN_VAL
                //Always check if the new point can be added or not
                // If not add another poisoning point, check repeat
                //IF we can now add the new point, add it and increase the points in hull
                // Change the poisoning point to false when trying to add a new point into the hull.
                //Can I just add that point ? No right, need a better way to add the point
                //I probably need another implementation

                max_y = std::numeric_limits<Y>::max();
                min_y = std::numeric_limits<Y>::lowest();

                //Get the rank of xp
                int yp = 0;
                p1{x, yp >= max_y - epsilon ? max_y : y + epsilon};
                p2{x, yp <= min_y + epsilon ? min_y : y - epsilon};

                slope1 = rectangle[2] - rectangle[0];
                slope2 = rectangle[3] - rectangle[1];
                outside_line1 = p1 - rectangle[2] < slope1;
                outside_line2 = p2 - rectangle[3] > slope2;
        ...
        //Check what this checks exactly
        //What is extreme slope
        //Check if there items change when I add a new point into the rectangle
        //IF SO do these
        // IF not great
        // Find out
        // How does the rectangle work
        // What is extreme slope
        // Does poisoning point change the rectangle 
        // What is the hull update
        //Adding a new point changes the rectangle becasue the ranks change
        // I need to just get the rectangle at the end somehow
...
size_t make_segmentation(size_t n, size_t epsilon, Fin in, Fout out, int level) {
    if (n == 0)
        return 0;
    ...
    opt.add_point(p.first, p.second, level);
    ...
    for (size_t i = 1; i < n; ++i) {
        ...
        if (next_p.first == p.first)
            continue;
        p = next_p;
        if (!opt.add_point(p.first, p.second, level)) {
            out(opt.get_segment());
            opt.add_point(p.first, p.second,level);
            ++c;
        }
...
template<typename Fin, typename Fout>
size_t make_segmentation_par(size_t n, size_t epsilon, Fin in, Fout out, int level) {
    ...
    if (parallelism == 1 || n < 1ull << 15)
        return make_segmentation(n, epsilon, in, out, level);
    ...
    for (auto i = 0; i < parallelism; ++i) {
        ...
        c += make_segmentation(last - first, epsilon, in_fun, out_fun, level);
    }
...
auto make_segmentation(RandomIt first, RandomIt last, size_t epsilon, int level) {
    ...
    make_segmentation(n, epsilon, in_fun, out_fun, level);
