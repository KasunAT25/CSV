...
#include "piecewise_linear_model_poi.hpp"
#include <algorithm>
#include <cstddef>
#include <cstdint>
#include <iterator>
...
public:
    ...
    static void build(RandomIt first, RandomIt last,
        ...
        std::cout << "Running the first level "<< std::endl;
        ...
        auto build_level = [&](auto epsilon, auto in_fun, auto out_fun, int level) {
            auto n_segments = internal::make_segmentation_par(last_n, epsilon, in_fun, out_fun, level);
        ...
        std::cout << "Right before the first level "<< std::endl;
        ...
        auto in_fun = [&](auto i) {
            ...
            //======================================
            //ISSUE IS HERE WITH THE INCREASING ONE, BECAUSE THEY ADJUST FOR DUPLICATES THEY INCREASE BY 1 
            //ISSUE IS THE FLAG IT SHOULD NOT BE 1 OR 0 IT SHOULD BE THE PRECISON OF THE DATASET (0.0001 ETC AND 0)
            //std::cout << "x before "<< x << " | ";
            auto flag = i > 0 && i + 1u < n && x == first[i - 1] && x != first[i + 1] && x + 1 != first[i + 1];
            //auto flag = i > 0 && i + 1u < n && x == first[i - 1] && x != first[i + 1] && x + 1*0.0001 != first[i + 1];

            // std::cout << "Flag "<< flag << " | ";
            // std::cout << "x after "<< x+flag*0.00001 <<std::endl;
            return std::pair<K, size_t>(x + flag, i);

            //return std::pair<K, size_t>(x + flag*0.0001, i);
        };
        auto out_fun = [&](auto cs) { segments.emplace_back(cs); };
        last_n = build_level(epsilon, in_fun, out_fun,level);
        ...
        while (epsilon_recursive && last_n > 1) {
            ...
            last_n = build_level(epsilon_recursive, in_fun_rec, out_fun,level);
            levels_offsets.push_back(levels_offsets.back() + last_n + 1);
        }
    }
