...
//  g++ -std=c++17 -I /opt/homebrew/Cellar/boost/1.82.0_1/include -L /opt/homebrew/Cellar/boost/1.82.0_1/lib -o my_program test.cpp -lboost_system
#include <iostream>
...
int main() {
    ...
    std::cout << "Pair values: " << init_data.size() << " and " << init_data[0].second << std::endl;
    ...
    std::cout << "Pair values: of the data" << init_data.size() << " and " << init_data[0].second << std::endl;
        std::cout << "Pair values: of the requests" << requests.size() << " and " << init_data[0].second << std::endl;
  ...
  const double conflicts_decay = 0.1;
    ...
    std::string filename = "results for books_200M_uint32 read only";
    ...
    for (int batch_idx = 0; batch_idx < num_batches; ++ batch_idx) {
      ...
      for(int q =0;q<q_num;q++){
        ...
        for (int i = l; i < r; ++ i) {
          ...
          time_i = 0;
          int data_idx = i - l;
          if (requests[i].op == kQuery && is_query) {
