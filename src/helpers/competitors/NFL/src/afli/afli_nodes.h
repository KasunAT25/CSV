...
public:
  ...
  ResultIterator<KT, VT> find(KT key) {
    if (model_ != nullptr) {
      uint32_t idx = std::min(std::max(model_->predict(key), static_cast<int64_t>(0)), 
  ...
  bool update(KVT kv) {
    if (model_ != nullptr) {
      uint32_t idx = std::min(std::max(model_->predict(kv.first), static_cast<int64_t>(0)), 
