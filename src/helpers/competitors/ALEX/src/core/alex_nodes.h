...
class AlexModelNode : public AlexNode<T, P> {
 public:
  ...
  int num_keys_model_ = 0;

  // Array of pointers to children
  AlexNode<T, P>** children_ = nullptr;

  // Children cost only if they are all data nodes otherwise 0
  ...
  bool validate_structure(bool verbose = false) const {
    ...
    if (cur_repeats != (1 << cur_child->duplication_factor_)) {
      if (verbose) {
        std::cout << "[Incorrect duplication factor2] num actual repeats: "
                  << cur_repeats << ", num dup_factor repeats: "
                  << (1 << cur_child->duplication_factor_)
                  << ", parent addr: " << this
                  << ", parent level: " << this->level_
...
class AlexDataNode : public AlexNode<T, P> {
 public:
  ...
  double expected_exp_search_iterations_ = 0;
  //double expected_exp_search_iterations_ = 0;
  ...
  inline void set_bit(int pos) {
    // std::cout << pos << " " << data_capacity_ << std::endl;
    assert(pos >= 0 && pos < data_capacity_);
    int bitmap_pos = pos >> 6;
    int bit_pos = pos - (bitmap_pos << 6);
  ...
  //THIS IS MY NEW COST METHOD
  //=====================================
  double compute_new_cost(
      const V* values, int num_keys, int current_keys, int parent_keys, short level, double density,double const_search,double const_traversal,
      double expected_insert_frac,
      const LinearModel<T>* existing_model = nullptr, bool use_sampling = false,
      DataNodeStats* stats = nullptr) {
    // if (use_sampling) {
    ...
    this->expected_exp_search_iterations_ = expected_avg_exp_search_iterations;
    //std::cout << expected_avg_exp_search_iterations << std::endl;
    
    //expected_exp_search_iterations = expected_avg_exp_search_iterations;

    //expected_exp_search_iterations = expected_avg_exp_search_iterations;
    // double sum_of_squared_errors = 0.0;
    // //ADD THE EQUATION VERSION
    // //int i =0;
    ...
    //         double squared_error = pow(model.predict(x) - y_actual, 2);
    ...
    //ADD Number of searches expected, time by level (traversal time), size difference
    //I used the below cost
    //============
   // cost = kExpSearchIterationsWeight * expected_avg_exp_search_iterations + kNodeLookupsWeight*level*0.5;

    //This is the more fine tuned cost (I need to change the constants)
    // cost = const_search * expected_avg_exp_search_iterations + const_traversal*level +
    // kExpSearchIterationsWeight * expected_avg_exp_search_iterations +
    //        kShiftsWeight * expected_avg_shifts * expected_insert_frac +  (kModelSizeWeight *
    //    (sizeof(AlexDataNode<T, P>) + sizeof(void*)) * num_keys);

    //   cost = 
    // kExpSearchIterationsWeight * expected_avg_exp_search_iterations  + //searches
    //   kNodeLookupsWeight +
      // (kModelSizeWeight *
      //  (sizeof(AlexDataNode<T, P>) + sizeof(void*)) * total_keys / num_keys) + //size
    //  kShiftsWeight * expected_avg_shifts * expected_insert_frac; // shifts

      // cost = (kExpSearchIterationsWeight * expected_avg_exp_search_iterations +
      //      kShiftsWeight * expected_avg_shifts * expected_insert_frac +
      //      (kModelSizeWeight * (sizeof(AlexDataNode<T, P>) + sizeof(void*)) *num_keys)+kNodeLookupsWeight*level)*num_keys + 
      //      kNodeLookupsWeight*level*num_keys;

  //COST 1
  //=========================
    cost = (const_search * expected_avg_exp_search_iterations +
           const_traversal*level)*(current_keys/(1.0*parent_keys)) ;

    // cost = (const_search * expected_avg_exp_search_iterations*kExpSearchIterationsWeight +
    //        const_traversal*level*kNodeLookupsWeight)*current_keys ;

  //COST 2
  //=========================
    // cost = (kExpSearchIterationsWeight * expected_avg_exp_search_iterations +
    //        kNodeLookupsWeight*level)*current_keys ;

    //COST 3
    //=========================
    // cost = (kExpSearchIterationsWeight * expected_avg_exp_search_iterations +
    //        kNodeLookupsWeight*level)*current_keys + 
    //        (kModelSizeWeight * (sizeof(AlexDataNode<T, P>) + sizeof(void*))) ;


    //COST 4
    //  cost = (const_search * expected_avg_exp_search_iterations +
    //        const_traversal*level)*current_keys +
    //        (kModelSizeWeight * (sizeof(AlexDataNode<T, P>) + sizeof(void*))) ;

    // cost = (kExpSearchIterationsWeight * expected_avg_exp_search_iterations +
    //        kNodeLookupsWeight*level +  kShiftsWeight * expected_avg_shifts * expected_insert_frac )*current_keys;
           
    // also add the total_keys one for the whole dataset. the number of keys for
    // traversal is the total number of keys for all the children 

    //cost = sum_of_squared_errors/num_keys + kNodeLookupsWeight*level;
  ...
  double compute_new_cost_new(
      const V* values, int num_keys, int current_keys, short level, double density,double const_search,double const_traversal,
      double expected_insert_frac,
      const LinearModel<T>* existing_model = nullptr, bool use_sampling = false,
      DataNodeStats* stats = nullptr) {
    // if (use_sampling) {
  ...
  //Basically create the needed stuff
  ...
  void bulk_load(const V values[], int num_keys,
    ...
    int last_position = -1;
    int keys_remaining = num_keys;
    //just like the compute costs get the predicted and actual and get the max
    for (int i = 0; i < num_keys; i++) {
      int position = this->model_.predict(values[i].first);
      position = std::max<int>(position, last_position + 1);

      //If not enough space
      //Then leave no gaps and fill the rest
    ...
    //Say at the end the rest are gaps
    ...
    // this->model_.a_ = 0;
    // this->model_.b_ = values[0].first;
  }

  // New bulk loading
  void bulk_load_new(const V values[], std::vector<KEY_TYPE> leg, int num_keys,
                 const LinearModel<T>* pretrained_model = nullptr,
                 bool train_with_sample = false) {
    initialize(num_keys, kInitDensity_);
    ...
    //std::cout<< "Data C " << data_capacity_ << std::endl ;
    //just like the compute costs get the predicted and actual and get the max
    for (int i = 0; i < num_keys; i++) {
      int position = this->model_.predict(values[i].first);
      position = std::max<int>(position, last_position + 1);
...
#if ALEX_DATA_NODE_SEP_ARRAYS
      //ranks.push_back(position);
      //std::cout << position << " " << keys_remaining << std::endl;
      key_slots_[position] = values[i].first;
      payload_slots_[position] = values[i].second;
      set_bit(position);
    ...
    //build_model_new(values, num_keys, &(this->model_), ranks, train_with_sample);
    // this->model_.a_ = 0;
    // this->model_.b_ = values[0].first;
    int idx2 = 0;
    int size_real = leg.size();

    for(int i =0;i < num_keys;i++){
      if(std::find(leg.begin(), leg.end(), values[i].first) == leg.end()){
        //erase(values[i].first);
        //unset_bit()
      }
      else{
  ...
  void bulk_load_new_rank(const V values[], std::vector<int> ranks, int num_keys,
                 const LinearModel<T>* pretrained_model = nullptr,
                 bool train_with_sample = false) {
    initialize(ranks[num_keys-1], kInitDensity_);
    ...
    //data_capacity_ = 100;
    std::cout<< "Data C " << data_capacity_;
    std::cout<< "Rank e " << ranks[num_keys-1];
    ...
    //If not create a new model
      ...
      //build_model(values, num_keys, &(this->model_), train_with_sample);
      build_model_new(values, num_keys, &(this->model_), ranks, false);
    }
    //expand it
    this->model_.expand(static_cast<double>(data_capacity_) / num_keys);
...
//
  ...
  static void build_model_new(const V* values, int num_keys, LinearModel<T>* model, std::vector<int> ranks,
                          bool use_sampling = false) {
    if (use_sampling) {
      build_model_sampling(values, num_keys, model);
      return;
...
//
    LinearModelBuilder<T> builder(model);
    for (int i = 0; i < num_keys; i++) {
      builder.add(values[i].first, ranks[i]);
    }
    builder.build();
  }
  ...
  inline int exponential_search_upper_bound(int m, const K& key) {
    ...
    // binary search.
    number_of_searches_ = 0;
    int bound = 1;
    int l, r;  // will do binary search in range [l, r)
    //std::cout << ALEX_DATA_NODE_KEY_AT(m) << " "<< key <<std::endl;
    //std::cout << (key_greater(ALEX_DATA_NODE_KEY_AT(m), key)) << std::endl;
    ...
    //std::cout << "searches " << number_of_searches_ << " , ";
     //std::cout << "a2 " << std::endl;
  ...
  template <class K>
  inline int binary_search_upper_bound(int l, int r, const K& key)  {
    while (l < r) {
