...
//Need to get an idea regarding this
/*
...
class Alex {
 ...
 public:
  ...
  //type of the value it takes
  typedef std::pair<T, P> V;

  // ALEX class aliases
  //Uses self_type to reference itself
  typedef Alex<T, P, Compare, Alloc, allow_duplicates> self_type;
  //This is the Model node (uses AlexNode template)
...
//I change from private to public
 private:
 ...
 public:
  ...
  //Creates the ALEX index (from an empty index)
  void bulk_load(const V values[], int num_keys) {
    ...
    //Creates a linear model to hold it
    LinearModel<T> root_data_node_model;
    //Creates the root model node (a basic regression) alex_base.h
    //why not use model_node_type
    //build_model is in alex_node but the Linear_model and Linear_model_builder is in alex_base.h
    data_node_type::build_model(values, num_keys, &root_data_node_model,
                                params_.approximate_model_computation);
    
    //This is where I compute the cost
    //Needs to expand the model using the density then calculates the cost
    // check the predicted and the "actual" position and calculate the error
    DataNodeStats stats;
 ...
 private:
  ...
  void bulk_load_node(const V values[], int num_keys, AlexNode<T, P>*& node,
    ...
    // than current cost
    // basically is the cost is less than the weight itself then the child costs has to be greater
    //Or if the slope is 0 meaning they all lie in the same line.
    if (num_keys <= derived_params_.max_data_node_slots *
      ...
      //THIS BULK LOAD IS DIFFERENT FROM THE ORIGINAL. THIS IS FOR THE NODE ONLY.
      //====================== 
      //bulk load it (basically create a data node)
      //basically create a new data node and set the locations. bitmaps and all that
      data_node->bulk_load(values, num_keys, data_node_model,
                           params_.approximate_model_computation);

      //just use the same cost as the previously calculated one
    ...
    if (experimental_params_.fanout_selection_method == 0) {
      ...
      //From alex_fanout_tree.h use to calculate the children and their costs
    ...
    //If the cost of cost of the children is less than the node cost (model cost) then use the childrens (basically divide it more)
    if (best_fanout_tree_cost < node->cost_ ||
      ...
      //create a new model_node
      auto model_node = new (model_node_allocator().allocate(1))
          model_node_type(node->level_, allocator_);

      //I ADDED THIS PART SO I CAN KEEP THE MODEL COST
      //==============================================
      model_node->cost_ = node->cost_;
      ...
      if (best_fanout_tree_depth == 0) {
        ...
        int max_data_node_keys = static_cast<int>(
            derived_params_.max_data_node_slots * data_node_type::kInitDensity_);
        //Calculates the total cost and the children of the model node (used_fanout_tree_nodes)
      ...
      //set the slope and intercept of this model node to the node we are checking *fanout 
      ...
      int cur = 0;
      //I might have to delete the children here ish
      for (fanout_tree::FTNode& tree_node : used_fanout_tree_nodes) {
        ...
        //Use something similar to change the old child node of model node to data node for my one
        model_node->children_[cur] = child_node;

        //Do the same (basically bulk_load_node) check if it is model or data and if so what are its children and so on
      ...
      delete_node(node);
      node = model_node;
    } else { //If it is suppose to be a data node
      ...
      auto data_node = new (data_node_allocator().allocate(1))
          data_node_type(node->level_, derived_params_.max_data_node_slots,
                         key_less_, allocator_);
      //bulk load it from the alex_node's bulk load
      data_node->bulk_load(values, num_keys, data_node_model,
                           params_.approximate_model_computation);
      data_node->cost_ = node->cost_;
      delete_node(node);
 ...
 public:
  ...
  int erase(const T& key) {
    data_node_type* leaf = get_leaf(key);
    int num_erased = leaf->erase2(key);
    stats_.num_keys -= num_erased;
    if (leaf->num_keys_ == 0) {
      merge(leaf, key);
    }
