...
void cal_loss(std::vector<KEY_TYPE> & original_keyset,std::vector<KEY_TYPE> & current_keyset2,std::vector<size_t>& rankset,
    ...
    for(int i = 0; i < S.size(); i++) {
        ...
        //Calculate the loss 
        // long double L =0;
        // for(KEY_TYPE k: current_keyset2){
        //     L += pow();
        // }
        // long double Lt =0;
        // std::unordered_map<int, int> predictionsCount;
        // for(KEY_TYPE k: original_keyset){
        //     // int prediction = int(static_cast<long double>(k)*m+b);
        //     // predictionsCount[prediction]++;

        //     int rank = compute_rank_for_endpoint(k,current_keyset2);
        //     if(yp<rank){
        //         rank++;
        //     }  
        //     Lt += pow(int(static_cast<long double>(k)*m+b) - rank,2);
        // }
        // int totalCollisions = 0;
        // for (const auto& entry : predictionsCount) {
        //     if (entry.second > 1) {
        //         totalCollisions += (entry.second - 1); // Count collisions
        //     }
        // }
        // L[i] = totalCollisions;
        //L[i] = Lt;
        ...
        //L[i] = (m*m*sumX2 +2*n*m*b*avgXo - 2*m*sumXY+n*b*b - 2*n*b*avgY + sumY2o + (n*n) - yp*yp );
...
std::set<KEY_TYPE> obtain_poisoning_keys(double poisoning_threshold, std::vector<KEY_TYPE> & keyset, std::vector<size_t> &rankset, bool max = false) {
    ...
    while(poisoning_keys.size() < P){
        ...
        if(poisoning_keys.size()==0){
            cal_loss(keyset,keyset2,rankset,
        L, 
        S,true, L_prev,
        ...
        if(maxElementIterator->second >= L_temp){
            if(count_tries == 0){
        ...
        else if(maxElementIterator->second < L_temp){
            count_tries = 0;
            L_temp = maxElementIterator->second;
        }
