...
# ps aux | head -n 1 && ps aux | grep ./benchmark_sali_insert
# ps aux | head -n 1 && ps aux | grep ./sali_poi2
    ...
    for insert_prop in "${insert_props[@]}"; do
        for poison_size in "${poison_sizes[@]}"; do
            for poison in "${poisons[@]}"; do
                ...
                #echo "=============="
                LD_LIBRARY_PATH=/opt/intel/oneapi/tbb/2021.12/lib:$LD_LIBRARY_PATH ./sali_poi2 "$dataset" 200000000 "$poison" "$poison_size" "$insert_prop"
            done
        done
    done 
done