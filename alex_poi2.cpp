// RUN using
//CHANGED ALEX.H ERASE() INSIDE TO ERASE2 FROM ERASE
// g++ alex_poi2.cpp -std=c++17 -o alex_poi2 -march=native -mpopcnt
// ./alex_poi2 ../../../ext/Data/fb_200M_uint64 1000000

// ./alex_poi2 test 1000000 0 0.8 1 -30
...
std::vector<model_node_type*> replace_parents2(alex::Alex<KEY_TYPE, PAYLOAD_TYPE> &index,model_node_type* model,data_node_type *new_data_node, std::vector<model_node_type*> *model_nodes_by_level, int *model_idx
,int *parent_model_idx);
...
int main(int argc, char *argv[]){
   ...
   //Check if the dataset has any duplicates
    ...
    std::string performance_output = "results/alex/alex5_insert_original_performance.csv";
    std::string structure_output = "results/alex/alex5_insert_original_structure.csv";

    if(poison){
        performance_output = "results/alex/alex5_insert_poisoned_performance.csv";
        structure_output = "results/alex/alex5_insert_poisoned_structure.csv";
    }
     std::string changed_data_output = "results/changed_data.bin";
     std::string output_model_csv = "results/alex/alex5_insert_model_output_good.csv";
   ...
   //Getting all the nodes from the index and the maximum height (default is 0 aka root node only)
        ...
        //while(inner_idx == 0){
            ...
            if(best_node->cost_diff_ > (cutoff) || true){
               ...
               //Add a constant that if there were no new points also save
                ...
                new_data_node->duplication_factor_ = best_node->duplication_factor_;
            ...
            //new_data_node.validate_structure();
            ...
            //parents = replace_parents2(index,best_node, new_data_node, model_nodes_by_level, &best_node_idx, &parent_model_idx);
    ...
    // index.validate_structure(true,false);
    //  int count_issues = 0;
    //  for(int k : bulk_load_indexes){
    //         data_node_type* ss = index.get_leaf(legitimate_data[k]);
    //         if(ss == nullptr){
    //             count_issues++;
    ...
    for(const auto& element : poisoned_data){
                int num = index.erase(element);
                if(num == 0){ 
                    // std::cout << "Zero " << idx << std::endl;
                    idx++;
                }
    }
    std::cout << "Erase " << idx << std::endl;
    ...
    for(int i = 1; i <= max_model_height+1 ; i++ ){
        ...
        if(level_data.size()>0){
            ...
            // benchmark_alex_real_seperate(index,lookup_keys_level, lookup_keys_level, "ALEX", dataset_name+"_lookup_level_"+std::to_string(i), 
    ...
    // int count_issues2 = 0;
    //  for(KEY_TYPE k : legitimate_data){
    //         int ss = index.count(k);
    //         if(ss <=0){
    //             count_issues2++;
                
    //         }
    //     }
    //     std::cout << "Issues " << count_issues2 << std::endl;
...
int *parent_model_idx){
    ...
    // node_type** model_children  = model->children_;
    // int model_num_child = model->num_children_;

    // for(int i=0; i < model_num_child; i++){
    //     data_node_type* model_child = static_cast<data_node_type*>(model_children[i]);
    //     index.delete_node(model_child);
    // }
    // std::vector<alex::Alex<KEY_TYPE, PAYLOAD_TYPE> ::TraversalNode> traversal_path;

    // auto leaf_dup = index.get_leaf(key, &traversal_path);

    // int path_pos = static_cast<int>(traversal_path.size()) - 1;
    // alex::Alex<KEY_TYPE, PAYLOAD_TYPE> ::TraversalNode tn = traversal_path[path_pos];
    ...
    //     index.correct_traversal_path(leaf, traversal_path, true);
    //   } else if (leaf_dup->next_leaf_ == leaf) {
    //     index.correct_traversal_path(leaf, traversal_path, false);
    //   } else {
    //     assert(false);
    //     return;
    //   }
