...
#include "src/helpers/lipp_benchmark.h"
...
LIPP<KEY_TYPE, PAYLOAD_TYPE> index2;
...
int main(int argc, char *argv[]){
    ...
    double original_poi_thres = std::stod(poi_size_str);

    KEY_TYPE orignal_P = static_cast<KEY_TYPE>(original_poi_thres*values_size);
    ...
    double poi_thres = std::stod(poi_size_str);
    ...
    std::string performance_output = output_folder+"results/lipp/rev2_lipp_original_performance.csv";
    std::string structure_output = output_folder+"results/lipp/rev2_lipp_original_structure.csv";

    if(smooth){
        performance_output = output_folder+"results/lipp/rev2_lipp_smooth_performance.csv";
        structure_output = output_folder+"results/lipp/rev2_lipp_smooth_structure.csv";
    }


    std::string changed_data_output = "results/changed_data.bin";

    std::string output_model_csv = output_folder+"results/lipp/rev2_lipp_model_output_good.csv";
...
std::vector<KEY_TYPE> lookup_keys_zipf;
    if(insert){
        ...
        lookup_keys_zipf = get_search_keys_zipf(bulk_vector, bulk_size, 1000000, seed);
         ...
         benchmark_lipp_real(index2,lookup_changed, lookup_changed, "LIPP", dataset_name+"_changed", poi_thres, performance_output,
         smooth,insert,orignal_P,insert_threshold);
        benchmark_lipp_real(index2,lookup_keys, lookup_keys, "LIPP", dataset_name+"_lookup", poi_thres, performance_output,
         smooth,insert,orignal_P,insert_threshold);
         benchmark_lipp_real(index2,lookup_keys_zipf, lookup_keys_zipf, "LIPP", dataset_name+ "_lookup_zipf", poi_thres, performance_output,
         smooth,insert,orignal_P,insert_threshold);
    ...
    size_t index_size_after =  index2.index_size(true,false);


    std::string structure_results = "LIPP;" + dataset_name + ";" + std::to_string(smooth) + ";"+ std::to_string(insert) + ";" +
          std::to_string(orignal_P)  + ";"  + std::to_string(insert_threshold) + ";" +std::to_string(legitimate_data.size()) +";"
        + std::to_string(num_total_smooth) +
        ";"+ std::to_string(original_poi_thres) + ";" + std::to_string(altered_data_size)+ ";" + std::to_string(altered_data_size)+  ";Data";
    ...
    std::string model_results = "LIPP;" + dataset_name + ";" + std::to_string(smooth) + ";"+ std::to_string(insert) + ";" +
          std::to_string(orignal_P)  + ";"  + std::to_string(insert_threshold) + ";" +std::to_string(legitimate_data.size()) +";"
        ...
        ";"+ std::to_string(original_poi_thres) ;
    ...
    if(insert){
        ...
        for(int j = 0; j < 5; j++){
            ...
            std::string structure_results2 = "LIPP;" + dataset_name + "_insert_"+std::to_string(j) +";" + std::to_string(smooth) + ";"+ std::to_string(insert) + ";" +
          std::to_string(orignal_P)  + ";"  + std::to_string(insert_threshold) + ";" +std::to_string(legitimate_data.size()) +";"
        + std::to_string(num_total_smooth);
            ...
            benchmark_lipp_real(index2,lookup_changed, lookup_changed, "LIPP", dataset_name+"_changed"+ "_insert_"+std::to_string(j), poi_thres, performance_output,
            smooth,insert,orignal_P,insert_threshold);
            benchmark_lipp_real(index2,lookup_keys, lookup_keys, "LIPP", dataset_name+"_lookup"+ "_insert_"+std::to_string(j), poi_thres, performance_output,
                smooth,insert,orignal_P,insert_threshold);
            // benchmark_lipp_real(index2,lookup_keys_zipf, lookup_keys_zipf, "LIPP", dataset_name+ "_lookup_zipf"+ "_insert_"+std::to_string(j), poi_thres, performance_output,
            // smooth,insert,orignal_P,insert_threshold);
            ...
            // benchmark_lipp_real(index2,insert_vector, insert_vector, "LIPP", dataset_name+ "_insert"+ "_insert_"+std::to_string(j), poi_thres, performance_output,
            // smooth,insert,orignal_P,insert_threshold);

            size_t index_size_after2 =  index2.index_size(true,false);
    ...
    if(benchmark){
        ...
        if(!insert){
            ...
            std::cout << "Index size " << index2.index_size(true,false) <<std::endl;
